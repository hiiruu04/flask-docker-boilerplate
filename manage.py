...
def create_initial_db():
    ...
    try:
        run_sql([f"CREATE DATABASE {os.getenv('APPLICATION_CONFIG')}"])
...
def test(filenames):
    ...
    cmdline = ["pytest", "-svv", "--cov=flaskr", "--cov-report=term-missing"]
...
def up(name):
    ...
    if not os.path.isfile(scenario_config_source_file):
        raise ValueError(f"File {scenario_config_source_file} doesn't exists")
    ...
    if not os.path.isfile(scenario_docker_source_file):
        raise ValueError(f"File {scenario_docker_source_file} doesn't exists")
    shutil.copy(scenario_docker_source_file, scenario_docker_file)
